...
export default class Canvas {
  ...
  private onChangeStart(template: string) {
    this.template = template
  ...
  private onChangeEnd(template: string) {
    if (template == 'home') {
      // this.createHome()
    } else {
      // this.destroyHome()
  ...
  private updateScale() {
    ...
    const height: number = this.camera ? 2 * Math.tan(fov / 2) * this.camera?.position.z : 0 //z = 5 is setted at this.createCamera
  ...
  public onTouchMove(event: TouchEvent | MouseEvent) {
    ...
    // if (this.home) {
    //   this.home.onTouchMove(positions)
    // }
  }

  public onTouchUp(event: TouchEvent | MouseEvent) {
